...
def ingredient_profile(ing, prod_list):
    """Build a UNIQUE, data-derived prose <p> for a single INCI ingredient.

    Every clause is synthesized from real fields on ``ing`` or aggregates of
    ``prod_list``. Fields that are empty for a given row are omitted rather than
    filled with a constant, so no two ingredients emit the same fixed sentence.
    All field names below are verified against samples/ingredients.csv and the
    prod_list dicts assembled in load_data().
    """
    e = html.escape
    inci_name = (ing.get('inci_name') or '').strip()
    common = (ing.get('common_name') or '').strip()
    cas = (ing.get('cas_number') or '').strip()
    func = (ing.get('primary_function') or '').strip()
    comedo = (ing.get('comedogenic_rating') or '').strip()
    allergen = (ing.get('is_common_allergen') or '').strip() == '1'
    fungal = (ing.get('is_fungal_acne_trigger') or '').strip() == '1'

    sentences = []

    # 1) Identity + CosIng functional class + CAS registry status.
    ident = f"<strong>{e(inci_name)}</strong>" if inci_name else "This compound"
    if common and common.lower() != inci_name.lower():
        ident += f" (declared on labels as {e(common)})"
    if func:
        s1 = f"{ident} is catalogued in INCIDB under the CosIng functional class of {e(func)}"
    else:
        s1 = f"{ident} is catalogued in INCIDB"
    if cas:
        s1 += f", registered under CAS number {e(cas)}."
    else:
        s1 += ", carried without a discrete CAS registry assignment."
    sentences.append(s1)

    # 2) Occurrence aggregates derived from this ingredient's prod_list.
    ...
    if n_prod:
        s2 = (f"Within the free INCIDB sample partition it is mapped to "
              f"{n_prod} indexed formulation" + ("s" if n_prod != 1 else ""))
        if cats:
            s2 += (f" spanning {len(cats)} product categor"
                   + ("ies" if len(cats) != 1 else "y")
        ...
        if ranked:
            ...
            if best_pos == 1:
                s2 += f", leading the ingredient declaration in {e(best_name)}" if best_name \
                    else ", leading the ingredient declaration in at least one product"
            elif best_name:
                s2 += f", climbing to concentration rank #{best_pos} in {e(best_name)}"
            else:
                s2 += f", reaching as high as concentration rank #{best_pos}"
        s2 += "."
        sentences.append(s2)

    # 3) Comedogenic reading (only when a rating is actually present).
    if comedo:
        try:
            cval = float(comedo)
        except ValueError:
        ...
        if cval is not None:
            if cval >= 3:
                sentences.append(
                    f"Its comedogenic rating of {e(comedo)}/5.0 sits toward the "
                    f"pore-clogging end of the scale, warranting a patch test for "
                    f"acne-prone skin.")
            elif cval == 0:
                sentences.append(
                    f"It carries a comedogenic rating of {e(comedo)}/5.0, "
                    f"indicating no expected pore-clogging activity.")
            else:
                sentences.append(
                    f"Its comedogenic rating of {e(comedo)}/5.0 reflects a low "
                    f"pore-clogging profile.")

    # 4) Regulatory / trigger flags -- only surfaced when a flag is actually set.
    if allergen and fungal:
        sentences.append(
            "Regulatory metadata additionally flags it as both an FDA MoCRA "
            "contact allergen and a Malassezia (fungal-acne) trigger.")
    elif allergen:
        sentences.append(
            "It is flagged as an FDA MoCRA contact allergen, so patch testing is "
            "advised for sensitive users.")
    elif fungal:
        sentences.append(
            "It is flagged as a Malassezia (fungal-acne) trigger, relevant when "
            "managing fungal-acne or seborrheic concerns.")
